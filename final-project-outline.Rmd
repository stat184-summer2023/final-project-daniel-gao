...
**General Dataset:**
The General dataset consists of generic data on each game. The dataset contains 5 variables: Game, ReleaseDate, Genre, TotalEarnings, OnlineEarnings, TotalPlayers, and TotalTournaments. There are a total of 631 cases. Each case represents a total earnings a game since the its release date.
```{r}
str(General)
```
**Historical dataset:**
...
```
**Result:** In the Genres in Esports table, there consist of a total of 12 total genres. Each genre varies in total earning and popularity based on total player count for all of each genres tournaments, The genre with the highest total earning is Multiplayer Online Battle Arena with a total earning $540,062,581.76 and a total player count of 22,955 players.
```{r}
general_desc(results = 1)
```

**Results**: The genre with the lowest total earning is Puzzle Game with a total earning of $74,576.89 with a total player count of 225 players. The differences between the top and bottom genres varies greatly. The difference between total earning and total players for Multiplayer Online battle Arena and Puzzle Game is $539,988,004.87 and 22,955, respectively. 
```{r}
general_asc(results = 1)
```
...
```{r}
General %>%
  select(Genre, TotalEarnings, TotalPlayers) %>%
  group_by(Genre) %>% 
  summarise(TotalEarnings = sum(TotalEarnings), #sum of total earnings per genre
              TotalPlayers = sum(TotalPlayers)) %>% #sum of total players per genre
  arrange(desc(TotalEarnings)) %>%
...
```
This graph depicts the earnings of genres for the last ten years.Throughout the years, there are rise in earnings from all genres, but genres such as Battle Royale, Mutiplayer Online Battle Arena, and First-Person Shooter had an exponential increase in earnings. Compared the other genres, these three genres make up a notable difference for the earnings of all of the genres.
...
```
This graph displays the total players genre based on years. Although Multiplayer Online Battle Arena has more total earnings, the genre with the most consistent highest total players is First-Person Shooters. In 2019 and 2023, there can be seen a rise in player count in the Battle Royale genre.

## Conclusion
The difference between video game genres varies based on earnings and player count. Throughout the years, there can be seen a 